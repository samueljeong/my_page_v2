...
초안 자료에 제시된 모든 지침과 구조를 따르되, 개요가 아닌 완성된 설교문 형식으로 작성하세요.

작성 형식:
- 서론-본론-결론 구조의 완전한 문단형 설교문
- 초안의 대지 설계를 따라 각 대지마다 상세하게 전개
- 각 대지: 성경 해석 + 구체적 예화 + 실천적 적용
- 청중에게 직접 말하는 듯한 자연스러운 문체
- 분량: 충분히 풍성하게 (최소 2000자 이상)"""

        # 사용자 메시지 구성
        user_content = f"""다음은 설교 초안 자료입니다. 이 자료의 모든 내용과 지침을 바탕으로 완성된 설교문을 작성해주세요.

{draft_content}

위 초안의 구조(대지 설계)를 따르되, 각 부분을 완전한 설교문 형식으로 풍성하게 작성하세요."""
