...
## ★★★ 최우선 규칙: EPISODE_SUMMARIES.md가 절대적 기준 ★★★

╔════════════════════════════════════════════════════════════════╗
║  EPISODE_SUMMARIES.md에 정의된 내용만 대본으로 작성한다.         ║
║  서머리에 없는 내용을 임의로 추가하면 즉시 반려된다.              ║
╚════════════════════════════════════════════════════════════════╝

### ★★★ STEP 0: 서머리 확인 및 출력 (필수) ★★★

**대본 작성 전, 반드시 아래 형식으로 서머리를 출력하라:**

```
═══════════════════════════════════════════════
[EP00X 서머리 확인]
═══════════════════════════════════════════════
제목: (EPISODE_SUMMARIES.md에서 복사)
핵심 사건: (EPISODE_SUMMARIES.md에서 복사)
등장인물: (EPISODE_SUMMARIES.md에서 복사)
무영 상태: (EPISODE_SUMMARIES.md에서 복사)
감정선: (EPISODE_SUMMARIES.md에서 복사)
훅/클리프행어: (EPISODE_SUMMARIES.md에서 복사)
이전 화 연결: (EPISODE_SUMMARIES.md에서 복사)

[본편 구조]
1. (EPISODE_SUMMARIES.md에서 복사)
2. (EPISODE_SUMMARIES.md에서 복사)
3. ...
═══════════════════════════════════════════════
```

**서머리 출력 없이 대본 작성 시작 금지!**

### STEP 1: 대본 작성 후 자체 검증 (필수)

대본 작성 완료 후, 아래 체크리스트 출력:

```
[EP00X 서머리 일치 검증]
□ 핵심 사건이 대본에 포함됨? → (예/아니오 + 근거)
□ 등장인물이 서머리와 일치? → (예/아니오 + 근거)
□ 감정선 흐름이 반영됨? → (예/아니오 + 근거)
□ 훅/클리프행어가 포함됨? → (예/아니오 + 근거)
□ 이전 화 마지막과 자연스럽게 연결? → (예/아니오 + 근거)
```

### 절대 금지 (위반 시 즉시 반려)
- ❌ EPISODE_SUMMARIES.md에 없는 사건/캐릭터 추가
- ❌ 서머리의 핵심 사건 누락
- ❌ 등장 예정 아닌 캐릭터 조기 등장
- ❌ 이전 에피소드와 설정 충돌
- ❌ 캐릭터 경지/성격 임의 변경
...
- 본편: 마을 도착 → 소통 실패 → 굶주림 → 카이든 등장 → 빵 건넴

**올바른 대본:** 마을 도착, 언어 안 통함, 카이든이 빵 건넴
**잘못된 대본:** 기사단 숙소, 칼리아/마르쿠스 등장 ← 서머리에 없음!
