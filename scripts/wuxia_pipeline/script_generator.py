...
SCRIPT_TARGET_LENGTH = SCRIPT_CONFIG.get("target_chars", 45000)    # 50분 분량
SCRIPT_MIN_LENGTH = SCRIPT_CONFIG.get("min_chars", 40000)          # 최소 44분
SCRIPT_MAX_LENGTH = SCRIPT_CONFIG.get("max_chars", 54000)          # 최대 60분
...
★★★ 핵심 목표: 50분 장편 오디오북 ★★★
════════════════════════════════════════
청취자가 "한 화만 더..."를 외치게 만드는 몰입형 장편 대본을 작성합니다.

★ 재미있는 무협 소설의 필수 요소:
1. **반전**: 매 챕터마다 예상을 뒤엎는 전개
2. **긴장감**: 주인공이 위기에 처하는 상황 빈번하게
3. **대결**: 무공 대결은 상세하게, 기술명과 내공 묘사 풍부하게
4. **감정**: 분노, 복수심, 의리, 은혜 등 강렬한 감정선
5. **클리프행어**: 매 화 끝에 "다음이 궁금해!" 유발
...
```json
{{{{
  "script": "[나레이션] 대본 내용... (40,000~54,000자 장편)",
  "chapters": [
    {{{{
      "chapter_number": 1,
