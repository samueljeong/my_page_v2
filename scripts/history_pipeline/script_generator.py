...
════════════════════════════════════════
• 종결: 친근한 구어체 종결어미 사용
  → ~였는데요, ~했거든요, ~였죠, ~이었어요, ~했어요
  → 친구에게 이야기 들려주듯 편안하게
  → "~습니다/입니다" 격식체는 딱딱하고 강연 같음

• 비율 (대략):
  - 60%: ~였는데요, ~했거든요, ~이었어요 (부드러운 연결)
  - 30%: ~였죠, ~잖아요 (청자와 공감대 형성)
  - 10%: ~였습니다 (강조할 때만, 포인트로)
...
✅ 좋은 예시 (편안한 구어체):
  "광개토왕은 즉위하자마자 군대를 일으켰거든요. 목표는 백제였어요. 한강을 움켜쥔 그 나라의 손을 풀어야 고구려의 숨통이 트이잖아요."
  "지난 시간에 신라 건국과 발전이 이어졌는데요. 오늘은 그 이후 이야기예요."
  "여기서 상황이 뒤집혔죠. 신라가 손을 내밀었거든요."
