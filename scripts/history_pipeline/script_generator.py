...
# ============================================================
# 대본 스타일 프롬프트 (방송용 다큐 나레이션 - 2025-01 개선)
# ============================================================
SCRIPT_STYLE_PROMPT = """당신은 한국어 역사 다큐 유튜브 채널의 최상급 대본 작가입니다.
...
0) 최우선 목표
════════════════════════════════════════
• 최종 산출물: 한글 약 20,000자(±10%)
• 스타일: 다큐 나레이션(방송 원고) — 장면감, 전환, 긴장감, 여운
• 톤: 과장 없이 몰입감 있게. 군더더기 없이 선명하게.
• 문장 호흡: 한 문장 15~35자 중심, 긴 문장은 두 문장으로 쪼개기.
• 중복 금지: 같은 사건을 "요약→본문→재요약"으로 반복하지 마세요. 한 번 말할 때 끝내고 다음으로 이동하세요.

════════════════════════════════════════
1) 절대 금지 (중요)
════════════════════════════════════════
• "한국민족문화대백과사전은 ~라고 설명한다" 같은 출처명 반복 문장 금지
  (출처는 글 속에서 반복하지 말고, 내용 자체로 신뢰를 확보하세요)
...
2) 필수 구성 (대본 흐름)
════════════════════════════════════════
아래 순서를 자연스럽게 녹여서 "이야기"로 쓰세요.

① 오프닝 (강한 후킹 20~30초 분량)
   • 한 장면처럼 시작: 연도·인물·위기 상황 제시
   • 질문 1~2개로 궁금증 유발

② 배경 (왜 이 사건이 시작됐는가)
   • 즉위 전 10~30년 맥락: 위기/개혁/왕권 흐름
   • 청자가 '이제 이해된다'고 느끼게 만들기

③ 본론 (연대기 전개, 사건마다 "의미"가 이어지게)
   • 사건은 "왜→어떻게→결과→다음 사건으로의 연결" 4박자로 처리
   • 지명/수치/연도는 꼭 필요할 때만. 너무 빽빽하게 넣지 않기.

④ 클라이맥스 (가장 큰 전환점 1개를 강하게)
   • 여기서 리듬을 빠르게, 장면을 선명하게

⑤ 후반 (사료의 가치와 한계, 논쟁 포인트 1~2개만 깊게)
   • 쟁점은 "왜 논쟁인지" 중심으로 짧고 강하게
   • "확정되지 않은 부분이 있다"는 태도를 보여 신뢰 확보

⑥ 엔딩 (여운 + 다음 편 연결)
   • "그가 남긴 것"을 한 문장으로 정리
   • 다음 에피소드 주제로 자연스럽게 넘어가기

════════════════════════════════════════
3) 글의 '맛' 규칙 (방송 퀄리티)
════════════════════════════════════════
• 문단 시작마다 장면 전환을 넣으세요.
  예: "겨울의 평양성." "강을 건넙니다." "신라의 사신이 도착합니다."
• 독자가 '머릿속으로 보게' 쓰세요.
  추상어만 쓰지 말고 "성벽, 강, 사신, 조공, 인질" 같은 구체 단서로 그리기.
• 다만 소설처럼 창작하지 말고, 사료 기반의 상상 가능한 범위만 허용.
...
def generate_script_by_parts(
    ...
    try:
        ...
        # ========================================
        print(f"[SCRIPT] Part 1: 인트로 생성 중 (이탈방지 + 에피소드 연결)...")
        intro_prompt = f"""당신은 한국사 전문 유튜브 채널의 최고 대본 작가입니다.
...
════════════════════════════════════════
★★★ 인트로 작성 지침 (API 장점 최대 활용) ★★★
════════════════════════════════════════

이 파트는 시청자가 영상을 이탈할지 결정하는 가장 중요한 30초입니다.

【API 장점 활용 - 에피소드 연결】
이전 에피소드 정보가 있다면 자연스럽게 연결하세요:
- 첫 화: "한국사의 시작, 고조선 이야기를 시작합니다."
- 2화 이후: "지난 시간, [이전 내용 한 줄 요약]... 오늘은 그 이후 이야기입니다."
- 시대 첫 화: "[이전 시대]가 저물고 새로운 시대가 열립니다."

★ 단, "지난 시간에" 문구는 첫 2-3문장에서만 사용하고 빠르게 본론으로 전환하세요.

아래 5가지 기법을 반드시 적용하세요:

【기법 1: Cold Open (콜드 오픈)】
가장 극적인 순간부터 시작하세요. 배경 설명 없이 바로 사건 속으로.
❌ "오늘은 광개토대왕에 대해 알아보겠습니다"
✅ "서기 391년, 열여덟 살의 왕이 첫 출정에 나섭니다. 그의 앞에는 백제의 대군이 기다리고 있었습니다."

【기법 2: Open Loop (열린 질문)】
첫 30초 안에 해결되지 않은 질문을 던지세요. 시청자는 답을 알기 위해 머뭅니다.
✅ "왜 이 어린 왕은 즉위하자마자 전쟁에 나섰을까요?"
✅ "기록에는 이렇게 적혀있습니다. 하지만 무언가 이상합니다."

【기법 3: Curiosity Gap (호기심 간극)】
"알려진 사실"과 "숨겨진 진실" 사이의 간극을 만드세요.
✅ "삼국사기에는 이렇게 기록되어 있습니다. 그런데 광개토대왕비에는 전혀 다른 이야기가 새겨져 있습니다."
✅ "우리가 알고 있는 이야기는 여기까지입니다. 하지만..."

【기법 4: Stakes Reveal (중요성 암시)】
이 사건이 왜 중요한지 먼저 알려주세요.
✅ "이 전투의 결과가 동아시아의 판도를 바꾸게 됩니다."
✅ "이 결정 하나가 이후 수백 년의 역사를 결정짓습니다."

【기법 5: Pattern Interrupt (예상 파괴)】
시청자의 예상을 깨는 문장으로 주의를 끌으세요.
✅ "왕이 아니었습니다. 적어도, 아직은."
✅ "승리했습니다. 하지만 아무도 축하하지 않았습니다."
...
2. 미스터리 제시 (300-600자)
   - Open Loop 또는 Curiosity Gap 적용
   - "왜?", "어떻게?", "무엇이?" 질문
   - 시청자가 궁금해할 핵심 포인트

3. Stakes 암시 (600-900자)
   - 이 사건의 역사적 중요성
   - "이 전투가... 이 결정이..."
   - 시청자가 왜 계속 봐야 하는지 이유

4. 에피소드 로드맵 (900-1500자)
   - 이 영상에서 다룰 내용 예고
   - "지금부터... 살펴보겠습니다"
   - 어떤 질문에 답을 얻을 수 있는지
...
• 출처 명시 금지: "~에 따르면", "~에 기록되어 있습니다" ❌
• 보고서 종결어 금지: "기록되어 있다", "언급된다", "설명된다" ❌
• 감정적 표현 금지: 놀랍다, 위대하다, 충격적 등 ❌
...
대본만 작성하세요. 다른 설명 없이 나레이션 대본만 출력하세요."""
        ...
        # ========================================
        print(f"[SCRIPT] Part 2: 배경 설명 생성 중 (리텐션 훅 포함)...")
        background_prompt = f"""[이전 파트 - 인트로]
...
════════════════════════════════════════
[작성 지시 - 배경 설명] ★ 스토리텔링 + 자료 기반
════════════════════════════════════════

★★★ 핵심 원칙 ★★★
- 수집된 자료에 있는 내용만 사용 (상상 금지)
- 출처 표기 금지 ("~에 따르면" ❌) → 출처는 유튜브 설명란에 별도 정리
- 마치 이야기를 들려주듯 자연스럽게 서술

【내용 구성】
1. 시대적 맥락 (1,000자)
   - 자료에 있는 시대 상황을 이야기처럼 서술
   - 연도, 왕, 주요 사건 자연스럽게 녹여서

2. 인물/세력 소개 (1,000자)
   - 등장인물을 이야기 속 캐릭터처럼 소개
   - 딱딱한 설명 ❌, 생생한 서술 ✅
...
[스타일 - 방송 원고]
• 문장 호흡: 15~35자, 긴 문장은 쪼개기
• 장면 전환: 문단마다 "성벽 위의 파수꾼.", "조정이 술렁입니다." 같은 시각적 전환
• 출처 반복 금지: "~에 따르면", "~라고 설명한다" ❌
• 보고서 종결어 금지: "기록되어 있다", "설명된다" ❌
• 감정 표현 금지
...
대본만 작성하세요."""
        ...
        # ========================================
        print(f"[SCRIPT] Part 3-1: 본론 전반부 생성 중 (리텐션 훅 포함)...")
        body1_prompt = f"""[이전 파트 마지막 부분]
{bg_result['text'][-500:]}
...
════════════════════════════════════════
[작성 지시 - 본론 전반부] ★ 스토리텔링 + 자료 기반
════════════════════════════════════════

★★★ 절대 원칙 ★★★
- 수집된 자료에 있는 내용만 사용 (상상 금지)
- 출처 표기 금지 ("~에 따르면" ❌) → 흐름을 끊지 않도록
- 마치 역사 다큐멘터리 나레이션처럼 이야기를 전달

【내용 구성 - 시간순 전개】
각 사건을 이야기처럼 풀어서:
- 연도/시기는 자연스럽게 녹여서 ("서기 391년, ~")
- 인물의 행동과 결정을 중심으로
- 원인 → 전개 → 결과를 드라마틱하게

【이탈 방지 - 3개 훅 삽입】
본론이 지루해지지 않도록 2,000자마다 훅:
...
[스타일 - 방송 원고]
• 문장 호흡: 15~35자, 긴 문장은 쪼개기
• 장면 전환: 문단마다 "강을 건넙니다.", "전장에 도착합니다." 같은 시각적 전환
...
대본만 작성하세요."""
        ...
        print(f"[SCRIPT] Part 3-2: 본론 후반부 생성 중 (리텐션 훅 포함)...")
        body2_prompt = f"""[이전 파트 마지막 부분]
{body1_result['text'][-500:]}

[수집된 자료 중 아직 다루지 않은 부분]
...
3. 다음 에피소드 예고 (1,000자)
   - 다음 이야기에 대한 기대감 조성
   - "다음 시간에는 더 극적인 이야기가 펼쳐집니다"
   - Open Loop로 마무리 (궁금증 남기기)

【마무리 훅】
영상 끝까지 보게 만드는 마무리:
✅ "이 선택이 이후 역사를 어떻게 바꿨을까요?"
✅ "다음 시간에는 더 놀라운 이야기가 기다리고 있습니다."
✅ "과연 그들의 결정은 옳았을까요? 여러분의 생각은 어떠신가요?"

[스타일 - 방송 원고]
• 문장 호흡: 15~35자, 긴 문장은 쪼개기
• 여운: "그가 남긴 것"을 한 문장으로 정리
• 출처 반복 금지: "~에 따르면" ❌
• 보고서 종결어 금지: "정리하면", "마무리하면" ❌
• 감정 표현 금지
