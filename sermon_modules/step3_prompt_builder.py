...
[1대지] (대지 제목)
★ 각 대지는 다음 5가지 요소를 포함해야 합니다:
① 도입: 이 대지에서 다룰 핵심 질문이나 주제 제시 (2-3문장)
② 본문 해설: 해당 성경 구절의 의미를 스토리텔링으로 풀어서 설명 (5-8문장)
③ 예화: 구체적인 인물, 상황, 감정이 담긴 이야기 (5-8문장)
④ 적용: "이번 주에 이렇게 해보세요" 같은 실천 가능한 제안 (3-5문장)
⑤ 전환: 다음 대지로 자연스럽게 연결하는 문장 (1-2문장)

[2대지] (대지 제목)
(동일하게 5가지 요소 포함)

[3대지] (대지 제목)
(클라이맥스 - 가장 강력하고 풍성하게, 5가지 요소 + 감동적 절정)
...
2. 각 대지는 풍성하게 (★ 매우 중요)
   ✗ 짧은 대지 (금지):
   "믿음은 떠남입니다. 아브라함은 떠났습니다. 우리도 떠나야 합니다."

   ✓ 풍성한 대지 (권장):
   각 대지에 최소 15-20문장 이상 작성하세요.
   - 도입 질문으로 시작
   - 본문을 천천히 풀어서 설명
   - 생생한 예화로 청중의 마음을 움직이고
   - 구체적인 적용으로 마무리

   대지가 짧으면 설교가 빈약해집니다. 충분히 풀어서 설명하세요!
